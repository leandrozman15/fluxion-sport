...
export default function ShopAdminPage() {
  ...
  const handleCreateProduct = async () => {
    ...
    toast({ title: "Producto Publicado", description: "El artículo ya es visible en la tienda de socios." });
  };

  const handleSaveEdit = () => {
    if (!editingProduct) return;
    updateDocumentNonBlocking(doc(db, "clubs", clubId, "shop_products", editingProduct.id), { ...editingProduct });
    setIsEditOpen(false);
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 animate-in fade-in duration-500">
      ...
      <div className="flex-1 space-y-8 pb-24 px-4 md:px-0">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-white/80 font-bold uppercase tracking-widest text-[10px] mt-1">{club?.name} • Control de catálogo y stock oficial.</p>
          </div>
            ...
            <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
              ...
              <DialogContent className="max-w-3xl bg-white border-none shadow-2xl rounded-[2.5rem]">
                <DialogHeader>
                  ...
                  <DialogDescription className="font-bold text-slate-500">Define las características, stock y fotos del artículo.</DialogDescription>
                </DialogHeader>
                <ScrollArea className="max-h-[70vh] pr-4">
                  <div className="space-y-8 py-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea value={newProduct.description} onChange={e => setNewProduct({...newProduct, description: e.target.value})} placeholder="Detalles técnicos del material..." className="border-2" />
                    </div>

                    {/* Talles y Stock */}
                    <div className="space-y-4 bg-slate-50 p-6 rounded-[2rem] border-2 border-dashed border-slate-200">
                      <div className="flex justify-between items-center">
                        <Label className="font-black text-xs uppercase tracking-widest text-primary">Variantes y Disponibilidad</Label>
                        <Button variant="ghost" size="sm" onClick={() => handleAddSize(false)} className="text-primary hover:bg-primary/5 font-black text-[10px] uppercase">
                        ...
                        {newProduct.sizes.map((s, i) => (
                          <div key={i} className="flex gap-4 items-end animate-in slide-in-from-left-2">
                            <div className="flex-1 space-y-1.5">
                              <Label className="text-[10px] uppercase text-slate-400">Talle/Etiqueta</Label>
                              <Input value={s.label} onChange={e => handleUpdateSize(i, 'label', e.target.value, false)} placeholder="Ej. XL, 38, Único" className="h-10 bg-white" />
                            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <Card key={p.id} className="group overflow-hidden border-none shadow-2xl bg-white/95 backdrop-blur-md rounded-[2rem] transition-all hover:-translate-y-1">
                  ...
                  <CardHeader className="p-6">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">Stock Total: {totalStock} unid.</span>
                    </div>
                  </CardHeader>
                  <CardFooter className="bg-slate-50/50 border-t p-4 flex justify-end gap-2">
                    ...
                    <Button 
                      ...
                      onClick={() => { if(confirm("¿Eliminar este producto?")) deleteDocumentNonBlocking(doc(db, "clubs", clubId, "shop_products", p.id)) }}
                    >
          ...
          {filteredProducts?.length === 0 && (
            <div className="col-span-full text-center py-32 border-2 border-dashed rounded-[2.5rem] bg-white/5 opacity-50">
              <Package className="h-16 w-16 mx-auto text-white mb-4 opacity-20" />
              <p className="text-white font-black uppercase tracking-widest text-xs">No hay productos en el catálogo</p>
            </div>
          )}
        </div>
      </div>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-3xl bg-white border-none shadow-2xl rounded-[2.5rem]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black text-slate-900">Editar Producto</DialogTitle>
          </DialogHeader>
          {editingProduct && (
            <ScrollArea className="max-h-[70vh] pr-4">
              <div className="space-y-8 py-6">
                ...
                <div className="space-y-4 bg-slate-50 p-6 rounded-[2rem] border">
                  <div className="flex justify-between items-center">
                    <Label className="font-black text-xs uppercase tracking-widest text-primary">Stock por Talles</Label>
                    ...
                    {editingProduct.sizes?.map((s: any, i: number) => (
                      <div key={i} className="flex gap-4 items-end">
                        <div className="flex-1">
                          <Input value={s.label} onChange={e => handleUpdateSize(i, 'label', e.target.value, true)} placeholder="Talle" className="h-10 bg-white" />
                        </div>
                        <div className="w-32">
                          <Input type="number" value={s.stock} onChange={e => handleUpdateSize(i, 'stock', parseInt(e.target.value), true)} className="h-10 bg-white" />
                        </div>
