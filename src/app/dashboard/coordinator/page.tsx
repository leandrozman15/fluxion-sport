  ...
  useEffect(() => {
    async function fetchCoordinatorContext() {
      ...
      try {
        ...
        setTotalPlayers(playersSnap.size);

        // 4. Divisions → teams → results + next match
        ...
        const recentResults = results.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 5);
